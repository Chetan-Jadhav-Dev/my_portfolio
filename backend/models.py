...
class About(db.Model):
    ...
    hero_top_skills = db.Column(
        db.String(500)
    )  # Comma-separated top 5 skills for hero section
...
class Analytics(db.Model):
    ...
    )  # 'page_view', 'section_view', 'project_click', 'link_click', etc.
...
class ActivityLog(db.Model):
    ...
    entity_type = db.Column(
        db.String(50), nullable=False
    )  # 'project', 'skill', 'experience', 'about', 'contact'
...
class BlogComment(db.Model):
    ...
    blog_id = db.Column(db.Integer, db.ForeignKey("blog.id"), nullable=False)
    parent_id = db.Column(
        db.Integer, db.ForeignKey("blog_comment.id"), nullable=True
    )  # For replies

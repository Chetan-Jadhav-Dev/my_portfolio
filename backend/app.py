...
def update_project(project_id):
    ...
    project.detailed_description = data.get(
    ...
    log_activity(
        ...
        project.title,
        admin_user,
        {"old": old_data, "new": project.to_dict()},
    )
...
def delete_project(project_id):
    ...
    return (
        jsonify(
            {"message": "Project deleted successfully", "deleted_data": project_data}
        ),
        200,
    )
...
def update_about():
    ...
    about.hero_short_description = data.get(
        "hero_short_description", about.hero_short_description
    )
    ...
    log_activity(
        ...
        about.name,
        admin_user,
        {"old": old_data, "new": about.to_dict()},
    )
...
def update_skill(skill_id):
    ...
    log_activity(
        ...
        skill.name,
        admin_user,
        {"old": old_data, "new": skill.to_dict()},
    )
...
def delete_skill(skill_id):
    ...
    log_activity(
        "delete", "skill", skill_id, skill_data.get("name"), admin_user, skill_data
    )

    return (
        jsonify({"message": "Skill deleted successfully", "deleted_data": skill_data}),
        200,
    )
...
def track_event():
    ...
    data = request.get_json()
    ip_address = (
        request.remote_addr or request.headers.get("X-Forwarded-For", "").split(",")[0]
    )
    ...
    db.session.commit()
    return (
        jsonify({"message": "Event tracked", "session_id": analytics.session_id}),
        201,
    )
...
def get_analytics_stats():
    ...
    # Section views
    section_views = (
        db.session.query(Analytics.section, func.count(Analytics.id).label("count"))
        .filter(Analytics.section.isnot(None))
    ...
    # Top projects
    top_projects = (
        db.session.query(Analytics.item_name, func.count(Analytics.id).label("count"))
        .filter(
            Analytics.event_type == "project_click", Analytics.item_name.isnot(None)
        )
        .group_by(Analytics.item_name)
        .order_by(desc("count"))
        .limit(10)
        .all()
    )
    ...
    visitors_by_country = (
        ...
        .filter(Analytics.country.isnot(None), Analytics.country != "Unknown")
    ...
    visitors_by_city = (
        ...
        .filter(Analytics.city.isnot(None), Analytics.city != "Unknown")
        .group_by(Analytics.city, Analytics.country)
        .order_by(desc("count"))
        .limit(20)
        .all()
    )
    ...
    recent_activity = (
        Analytics.query.filter(Analytics.timestamp >= yesterday)
    ...
    hourly_traffic = (
        ...
        .filter(Analytics.timestamp >= yesterday)
        .group_by("hour")
        .order_by("hour")
        .all()
    )
        ...
        jsonify(
            {
                ...
                "section_views": [
                    {"section": s[0], "count": s[1]} for s in section_views
                ],
                "top_projects": [{"name": p[0], "count": p[1]} for p in top_projects],
                "visitors_by_country": [
                    {"country": c[0], "count": c[1]} for c in visitors_by_country
                ],
                "visitors_by_city": [
                    {"city": c[0], "country": c[1], "count": c[2]}
                    for c in visitors_by_city
                ],
                "recent_activity": [a.to_dict() for a in recent_activity],
                "hourly_traffic": [
                    {"hour": int(h[0]), "count": h[1]} for h in hourly_traffic
                ],
            }
        ),
        200,
...
def get_realtime_stats():
    ...
    recent_visitors = (
        db.session.query(func.count(func.distinct(Analytics.session_id)))
        .filter(Analytics.timestamp >= one_hour_ago)
        .scalar()
    )

    recent_views = Analytics.query.filter(Analytics.timestamp >= one_hour_ago).count()
        ...
        jsonify(
            {
                ...
                "recent_locations": [
                    {"city": l[0], "country": l[1], "count": l[2]}
...
def get_experience():
    experiences = Experience.query.order_by(
        Experience.order.desc(), Experience.start_date.desc()
    ).all()
...
def create_experience():
    ...
    exp = Experience(
        ...
        detailed_description=data.get("detailed_description"),
        technologies=(
            ",".join(data.get("technologies", [])) if data.get("technologies") else None
        ),
    ...
    # Log activity
    log_activity(
        "create", "experience", exp.id, f"{exp.position} at {exp.company}", admin_user
    )
...
def update_experience(exp_id):
    ...
    exp.detailed_description = data.get(
...
def delete_experience(exp_id):
    ...
    log_activity(
        ...
        "experience",
        exp_id,
        f"{exp_data.get('position')} at {exp_data.get('company')}",
        admin_user,
        exp_data,
    )
    ...
    return (
        jsonify(
            {"message": "Experience deleted successfully", "deleted_data": exp_data}
        ),
        200,
    )
...
{contact.message}
    ...
    return (
        jsonify({"message": "Contact form submitted successfully", "id": contact.id}),
        201,
    )
...
def delete_contact(contact_id):
    ...
    log_activity(
        ...
        "contact",
        contact_id,
        f"Contact from {contact_data.get('name')}",
        admin_user,
        contact_data,
    )
    ...
    return (
        jsonify(
            {"message": "Contact deleted successfully", "deleted_data": contact_data}
        ),
        200,
    )
...
def get_activity_logs():
    ...
    activities = (
        ActivityLog.query.order_by(desc(ActivityLog.timestamp)).limit(limit).all()
    )
...
def undo_activity(activity_id):
    ...
    try:
        snapshot = (
            json.loads(activity.data_snapshot)
            if isinstance(activity.data_snapshot, str)
            else activity.data_snapshot
        )
        ...
        if activity.entity_type == "project":
            project = Project(
                ...
                screenshots=(
                    json.dumps(snapshot.get("screenshots", []))
            ...
            exp = Experience(
                ...
                detailed_description=snapshot.get("detailed_description"),
                technologies=(
                    ",".join(snapshot.get("technologies", []))
        ...
        # Log undo action
        log_activity(
            "undo", activity.entity_type, restored_id, activity.entity_name, admin_user
        )
...
def update_github_settings():
    ...
    if "selected_repos" in data:
        ...
        if isinstance(selected_repos, str):
            ...
            settings.selected_repos = (
                json.dumps(selected_repos) if selected_repos else json.dumps([])
            )
    ...
    log_activity(
        ...
        "GitHub Settings",
        admin_user,
        {"old": old_data, "new": settings.to_dict()},
    )
...
def fetch_github_repos():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return (
                jsonify({"message": f"GitHub API error: {response.status_code}"}),
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for repo in all_repos:
                ...
                if (
                    repo_identifier in selected_repos
                    or repo.get("name") in selected_repos
                ):
                    formatted_repos.append(
                        {
                            "id": repo["id"],
            ...
            if response.status_code == 401:
                error_msg = (
                    "Invalid GitHub token. Please check your token in admin settings."
                )
...
def create_blog():
    ...
    try:
        ...
        slug = (
            data.get("slug")
            or re.sub(r"[^\w\s-]", "", data.get("title", "")).strip().lower()
        )
...
def delete_blog(blog_id):
    ...
    # Log activity
    log_activity(
        "delete", "blog", blog_id, blog_data.get("title"), admin_user, blog_data
    )

    return (
        jsonify({"message": "Blog deleted successfully", "deleted_data": blog_data}),
        200,
    )
...
def like_blog(blog_id):
    ...
    like = BlogLike(blog_id=blog_id, user_ip=user_ip, user_agent=user_agent)
    db.session.add(like)
    db.session.commit()
...
def get_blog_comments(blog_id):
    ...
    comments = (
        BlogComment.query.filter_by(blog_id=blog_id, approved=True, parent_id=None)
    ...
    for comment in comments:
        ...
        replies = (
            BlogComment.query.filter_by(parent_id=comment.id, approved=True)
            .order_by(BlogComment.created_at.asc())
            .all()
        )
...
def get_notifications():
    ...
    recent_likes = (
        BlogLike.query.filter_by(read=False)
        .order_by(BlogLike.created_at.desc())
        .limit(10)
        .all()
    )
    recent_comments = (
        BlogComment.query.filter_by(read=False, approved=True)
